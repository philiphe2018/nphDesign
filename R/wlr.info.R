...
#' F.entry = F.entry, G.ltfu = function(t){0}, n = 450)
#' 
#' #Draw plot of information for log-rank test and FH01 for T in (0, 48)
#' maxT = 48
...
wlr.info = function(T = c(24,36), r = 1, n = 450, h0 = function(t){log(2)/12}, S0= function(t){exp(-log(2)/12 * t)}, 
          ...
          rho1 = 0, gamma1 = 0, tau1 = NULL, s.tau1 = 0, f.ws1 = NULL,
          rho2 = 0, gamma2 = 0, tau2 = NULL, s.tau2 = 0, f.ws2 = NULL,
  ...
  #Weight function
  f.w = function(t, f.S = S0, f.ws=f.ws1, tau=tau1, s.tau=s.tau1, rho=rho1, gamma=gamma1){
    s = f.S(t)
    #First priority: f.ws
    if(!is.null(f.ws)){
      w = f.ws(s)
  ...
  f.cov = function(f = f0, f.S = S0) {
   ...
   if(length(T)==1 || (length(T)==2 && T[2] == T[1])){
    ...
    if(is.null(f.ws2) && (is.null(rho2) || is.null(gamma2))) {
      I11 = function(t){
        w1 = f.w(t, f.S = f.S, f.ws=f.ws1, tau=tau1, s.tau=s.tau1, rho=rho1, gamma=gamma1)
        return(w1^2 * F.entry(T[1]-t) * (1 - G.ltfu(t)) * f(t))
      }
      sigma11 = r1*r0*integrate(I11, lower=0, upper=T[1])$value
      cov = sigma11
      out = list()
      out$cov = cov
      ...
      I12 = function(t){
        w1 = f.w(t, f.S = f.S, f.ws=f.ws1, tau=tau1, s.tau=s.tau1, rho=rho1, gamma=gamma1)
        w2 = f.w(t, f.S = f.S, f.ws=f.ws2, tau=tau2, s.tau=s.tau2, rho=rho2, gamma=gamma2)
      ...
      sigma11 = r1*r0*integrate(I11, lower=0, upper=T[1])$value
      sigma12 = r1*r0*integrate(I12, lower=0, upper=T[1])$value
      sigma22 = r1*r0*integrate(I22, lower=0, upper=T[1])$value
  ...
  if (length(T)==2 && T[2] > T[1]) {
    ...
    if(is.null(f.ws2) && (is.null(rho2) || is.null(gamma2))){
      ...
      sigma11 = r1*r0*integrate(I11, lower=0, upper=T[1])$value
      sigma12 = sigma11
      sigma22 = r1*r0*integrate(I22, lower=0, upper=T[2])$value
    } else {
  ...
  cov.info.H1 = f.cov(f = f.bar, f.S = S.bar)
  cov.H1 = cov.info.H1$cov; info.H1 = cov.info.H1$info
  corr.H1 = NULL
  if(is.matrix(cov.H1)) {corr.H1 = cov.H1[1,2]/sqrt(cov.H1[1,1]*cov.H1[2,2])}
